...
if __name__ == '__main__':
    ...
    [np.random.shuffle(x) for x in training_data]
    training_labels = training_data[:, -1]
    training_data = training_data[:, :-1]
    training_data = training_data.reshape((60000, 784, 1))
    training_data = training_data.reshape((784, 60000, 1))
    ...
    model = slim.RNN(1, hidden_size = 500, num_layers = 2)
    for batch in range(0, 50000, 20):
        model.zero_grad()
        output, h_n = model(training_data[:, batch, :])
        true_labels = training_labels[batch]
        loss = criterion(output, true_labels)
        loss.backward()

    #validation test
    output, h_n = model(validation)
    loss = criterion(output, validation_labels)
    print("val loss: ", loss)
